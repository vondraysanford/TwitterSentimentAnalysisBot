...
class DiscordV2Bot(commands.Bot):
    ...
    async def import_public_key(self, filename):
        with open(filename, 'rb') as pem_in:
            pemlines = pem_in.read()
        public_key = serialization.load_pem_public_key(pemlines, default_backend())
        return public_key

    async def import_signature(self, filename):
        with open(filename, 'rb') as sig_in:
            sig_lines = sig_in.read()
        signature = base64.urlsafe_b64decode(sig_lines)
        return signature

    async def verify_model(self, bytes):
        try:
            public_key = await self.import_public_key('Resources/pubkey.cer')
            signature = await self.import_signature('Resources/signature.sig')
    ...
    async def get_xbg_model(self):
        file = open('Resources/model.pickle', 'rb')
        bytes = file.read()
        is_verified = await self.verify_model(bytes)
    ...
    async def get_bot_probability(self, user_features, xgb_model):
    ...
    async def get_result(self, probability):
        if probability < 20:
            return 'Not a bot'
        elif probability < 35:
    ...
    async def get_tweets_by_keyword(self, client, keyword):    
        tweet_ids = []
        tweet_times = []
        tweet_strings = []
        polarity_scores = []
        ...
        async for tweet in AsyncPaginator(client.search_recent_tweets, query=keyword, expansions = "author_id",  tweet_fields=['context_annotations', 'created_at'], max_results=100).flatten(limit=450):
            ...
            if ("RT @" not in tweet.text):
                ...
                account_features = await self.get_user_info(client, tweet.author_id)
                xgb_model = await self.get_xbg_model()
                probability_score = await self.get_bot_probability(account_features, xgb_model)
                option_status = await self.get_result (probability_score)
    ...
    async def get_user_info(self, client, id):
    ...
    async def get_sentiment(self, dataframe):
