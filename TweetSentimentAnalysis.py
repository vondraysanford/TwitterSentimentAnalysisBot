...
def get_tweets_by_keyword(client, keyword):    
    ...
    for tweet in tweepy.Paginator(client.search_recent_tweets, query=keyword, expansions = "author_id",  tweet_fields=['context_annotations', 'created_at'], max_results=100).flatten(limit=1000):
...
def get_sentiment(dataframe):
    ...
    # Get the average of the polarity column
